...
3. **RP capture** — `commemul.c` (ported from md-framebuffer-template) runs a second PIO state machine that samples the address of **every ROM3 read** into a 4 KB DMA ring (`COMM_RING_BITS 12`, 2048 samples) with no interrupt involved. ROM4 (`init_romemul(NULL, NULL, false)`) installs **no DMA IRQ at all** any more. The previous design captured `$FAF000+x` reads on ROM4 through a per-read DMA-completion IRQ that read back the channel's current address; two reads closer together than the IRQ latency overwrote each other, which lost key events and left keys stuck (EPIC-08 STORY-01).
