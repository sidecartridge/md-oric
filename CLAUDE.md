...
2. **Core 1 then toggles `sys->fb_toggle`** and writes `0x00000000` / `0xFFFFFFFF` to offset `0x0FFC` of the window. That longword is both the dirty flag and the page selector.
3. **The m68k VBL loop** (`.loop_low_st` in `main.s`) waits for the VBL flag, drains the AY queue, then compares `(ROM4_ADDR + $0FFC)` against its saved copy. Unchanged → go back to sleep. Changed → run the unrolled copy loop.
...
5. **The m68k flips the video base** (`$FFFF8201`/`$8203`) to the page it just filled. Which page is chosen by the toggle longword, so RP and ST agree on the page without a handshake. Known issue (see README roadmap): the displayed page is one frame behind.
...
5. **Function keys are intercepted** in `kbd_raw_key_down` before reaching the Oric matrix. `kbdmap_initOric()` (called once from `oric_main`) patches the table so ST **F1–F10** (`$3B`–`$44`) → `0x13A`–`0x143`, ST **UNDO** (`$61`) → `0x144`, ST **HELP** (`$62`) → `0x145`, plus the four arrow keys and a small set of Ctrl combinations the Oric firmware needs. `0x13A`–`0x143` insert a NIB disk image or a tape image by index, `0x144` raises NMI (README calls this "pause"), `0x145` resets the Oric.
...
- **Cart code must stay under `$0FFC`** — that is where the frame-toggle longword lives, and the framebuffer starts right after at `$1000`. This is the real size budget, not the 64 KB window.
- **No Atari RAM allocation**: the code uses no `.bss` and no heap. Its handful of variables (`.vblank_flag`, `.refresh_fb_flag`, `.overscan_flag`, `AYBUFF_POS`) live inside the copied code block itself, addressed through A6.
...
| `0x0FFC` | `$FA0FFC` | (`ROM4_ADDR + COPIED_CODE_SIZE - 4`) | 4 B | Frame toggle / dirty flag. RP writes `0` or `$FFFFFFFF` after each `oric_screen_update`; m68k re-blits only on change and uses the value to pick `$60000` vs `$70000`. |
