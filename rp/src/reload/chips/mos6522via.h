...
typedef struct {
  uint16_t
      latch;  // 16-bit initial value latch, NOTE: T2 only has an 8-bit latch
...
#define _MOS6522VIA_PIP_TEST(pip, offset, pos) \
  (0 != (pip & (1 << (offset + pos))))
...
static inline void _mos6522via_set_intr(mos6522via_t* c, uint8_t data) {
  c->intr.ifr |= data;
}
...
static inline void _mos6522via_clear_pa_intr(mos6522via_t* c) {
  _mos6522via_clear_intr(
      c, MOS6522VIA_IRQ_CA1 |
             (MOS6522VIA_PCR_CA2_IND_IRQ(c) ? 0 : MOS6522VIA_IRQ_CA2));
}

static inline void _mos6522via_clear_pb_intr(mos6522via_t* c) {
  _mos6522via_clear_intr(
      c, MOS6522VIA_IRQ_CB1 |
             (MOS6522VIA_PCR_CB2_IND_IRQ(c) ? 0 : MOS6522VIA_IRQ_CB2));
}
...
static void __not_in_flash_func(_mos6522via_tick_t1)(mos6522via_t* c,
                                                     uint8_t cycles) {
...
static void __not_in_flash_func(_mos6522via_tick_t2)(mos6522via_t* c,
                                                     uint8_t cycles) {
...
static void __not_in_flash_func(_mos6522via_update_cab)(mos6522via_t* c) {
...
inline bool _mos6522via_update_irq(mos6522via_t* c) {
...
// Perform a tick
bool __not_in_flash_func(mos6522via_tick)(mos6522via_t* c, uint8_t cycles) {
...
void mos6522via_write(mos6522via_t* c, uint8_t reg, uint8_t data) {
  switch (reg) {
    ...
    case MOS6522VIA_REG_ACR:
      ...
      */
      /* FIXME(?) this properly transitions T2 from counting PB6 to clock
       * counter mode */
      if (!MOS6522VIA_ACR_T2_COUNT_PB6(c)) {
...
inline uint8_t mos6522via_get_pa(mos6522via_t* c) {
  return (c->pa.inpr & ~c->pa.ddr) | (c->pa.outr & c->pa.ddr);
}

inline void mos6522via_set_pa(mos6522via_t* c, uint8_t data) {
...
void mos6522via_set_ca1(mos6522via_t* c, bool state) {
  c->pa.c1_triggered =
      (c->pa.c1_in != state) && ((state && MOS6522VIA_PCR_CA1_LOW_TO_HIGH(c)) ||
                                 (!state && MOS6522VIA_PCR_CA1_HIGH_TO_LOW(c)));
  c->pa.c1_in = state;
}

inline bool mos6522via_get_ca2(mos6522via_t* c) { return c->pa.c2_out; }

void mos6522via_set_ca2(mos6522via_t* c, bool state) {
  c->pa.c2_triggered =
      (c->pa.c2_in != state) && ((state && MOS6522VIA_PCR_CA2_LOW_TO_HIGH(c)) ||
                                 (!state && MOS6522VIA_PCR_CA2_HIGH_TO_LOW(c)));
  c->pa.c2_in = state;
}

uint8_t mos6522via_get_pb(mos6522via_t* c) {
  return _mos6522via_merge_pb7(
      c, (c->pb.inpr & ~c->pb.ddr) | (c->pb.outr & c->pb.ddr));
}
...
void mos6522via_set_cb1(mos6522via_t* c, bool state) {
  c->pb.c1_triggered =
      (c->pb.c1_in != state) && ((state && MOS6522VIA_PCR_CB1_LOW_TO_HIGH(c)) ||
                                 (!state && MOS6522VIA_PCR_CB1_HIGH_TO_LOW(c)));
  c->pb.c1_in = state;
}

inline bool mos6522via_get_cb2(mos6522via_t* c) { return c->pb.c2_out; }

void mos6522via_set_cb2(mos6522via_t* c, bool state) {
  c->pb.c2_triggered =
      (c->pb.c2_in != state) && ((state && MOS6522VIA_PCR_CB2_LOW_TO_HIGH(c)) ||
