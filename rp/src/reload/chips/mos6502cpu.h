...
//
// NOTE: this file is code-generated from mos6502cpu.template.h and
// mos6502cpu_gen.py in the 'codegen' directory.
//
// Do this:
// ~~~C
// #define CHIPS_IMPL
...
//         mos6510cpu_in_t in_cb;           // only mos6510cpu: port IO input
//         callback mos6510cpu_out_t out_cb;         // only mos6510cpu: port IO
//         output callback uint8_t mos6510cpu_io_pullup;    // only mos6510cpu:
//         IO port bits that are 1 when reading uint8_t mos6510cpu_io_floating;
//         // only mos6510cpu: unconnected IO port pins void*
//         mos6510cpu_user_data;      // only mos6510cpu: optional in/out
//         callback user data } mos6502cpu_desc_t;
//         ~~~
//
...
// there.
//
// mos6502cpu_init() will return a 64-bit pin mask which must be the input
// argument to the first call of mos6502cpu_tick().
//
// To execute instructions, call mos6502cpu_tick() in a loop. mos6502cpu_tick()
// takes a 64-bit pin mask as input, executes one clock tick, and returns a
// modified pin mask.
//
...
// sequence starts.
//
// To request an interrupt, set the MOS6502CPU_IRQ or MOS6502CPU_NMI bits in the
// pin mask and continue calling the tick function. The interrupt sequence will
// be initiated at the end of the current or next instruction (depending on the
// exact cycle the interrupt pin has been set).
//
...
//         ~~~C
//         typedef struct {
//             bool bcd_disabled;              // set to true if BCD mode is
//             disabled mos6510cpu_in_t mos6510cpu_in_cb;         // mos6510cpu
//             only: optional port IO input callback mos6510cpu_out_t
//             mos6510cpu_out_cb;       // mos6510cpu only: optional port IO
//             output callback void* mos6510cpu_user_data;          //
//             mos6510cpu only: optional callback user data uint8_t
//             mos6510cpu_io_pullup;        // mos6510cpu only: IO port bits
//             that are 1 when reading uint8_t mos6510cpu_io_floating;      //
//             mos6510cpu only: unconnected IO port pins
//         } mos6502cpu_desc_t;
//         ~~~
//
//     To emulate a mos6510cpu you must provide port IO callbacks in
//     mos6510cpu_in_cb and mos6510cpu_out_cb, and should initialize the
//     mos6510cpu_io_pullup and mos6510cpu_io_floating members to indicate which
//     of the IO pins are connected or hardwired to a 1-state.
//
// ~~~C
...
//     may call the input/output callback functions provided in
//     mos6502cpu_desc_t.
//
// ~~~C
...
typedef struct {
  bool bcd_disabled;  // Set to true if BCD mode is disabled
  mos6510cpu_in_t
      mos6510cpu_in_cb;  // Optional port IO input callback (only on mos6510cpu)
  mos6510cpu_out_t mos6510cpu_out_cb;  // Optional port IO output callback (only
                                       // on mos6510cpu)
...
static inline uint8_t _get_flags(mos6502cpu_t* c) {
  return (c->cf ? 0x01 : 0) | (c->zf ? 0x02 : 0) | (c->iflag ? 0x04 : 0) |
         (c->df ? 0x08 : 0) | (c->bf ? 0x10 : 0) | (c->xf ? 0x20 : 0) |
         (c->vf ? 0x40 : 0) | (c->nf ? 0x80 : 0);
}
...
// Only call this when accessing address 0 or 1 (MOS6510CPU_CHECK_IO(c)
// evaluates to true)
void __not_in_flash_func(mos6510cpu_iorq)(mos6502cpu_t* c) {
  ...
  if ((c->addr & 1) == 0) {
    ...
    if (c->rw) {
      ...
      c->out_cb((c->io_out & c->io_ddr) | (c->io_pullup & ~c->io_ddr),
                c->user_data);
    ...
    if (c->rw) {
      ...
      uint8_t val =
          ((c->io_inp | (c->io_floating & c->io_drive)) & ~c->io_ddr) |
          (c->io_out & c->io_ddr);
...
#pragma warning( \
    disable      \
    : 4244)  // Conversion from 'uint16_t' to 'uint8_t', possible loss of data
#endif

void __not_in_flash_func(mos6502cpu_tick)(mos6502cpu_t* c) {
  if (c->sync || c->irq || c->nmi || c->rdy || c->res) {
    ...
    if (c->sync) {
      // Load new instruction into 'instruction register' and restart tick
      // counter
      c->IR = _GD() << 3;
      c->sync = false;
